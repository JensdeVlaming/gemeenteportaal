...
import { getStoredOtpId, verifyOtpCode } from "@/services/authService";
import { Mail } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";

const OTP_LENGTH = Number(import.meta.env.VITE_OTP_LENGTH ?? 8) || 8;
...
export default function MagicLinkFlow() {
  ...
  const stateOtpId = (location.state as LocationState)?.otpId ?? "";
  const storedOtpId = getStoredOtpId(initialEmail);
  const otpId = stateOtpId || storedOtpId;
  ...
  async function handleVerifyOtpCode(digits?: string[]) {
    if (!hasEmail || !hasOtpId) {
      setOtpError(
        "Ga terug naar het inlogscherm om een login-code aan te vragen."
      );
      return;
    }
    ...
    try {
      await verifyOtpCode(otpId, joinedCode);
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white p-10 shadow-xl ring-1 ring-gray-100">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            We hebben een verificatiecode gestuurd naar {emailDisplay}. Vul de
        ...
        <div className="mt-6 rounded-2xl border border-[#E98C00] bg-[#fff7ef] p-6 text-sm text-[#1F2A44] shadow-sm">
          ...
          <p className="mt-2 text-sm leading-relaxed">
            Je hebt een login-code ontvangen in je e-mail. Vul de {OTP_LENGTH}
            -cijferige code hieronder in om je login te bevestigen.
          </p>
        </div>
